...
inside `draft` and `editor`, on every anchor the drafted prose cites — so
running it by hand here shows exactly what those automatic passes check.

A `PASS` on a `quote` or `page` claim — the two are the same check,
`--anchor` a page number either way (`verify.py:53`) — means one specific
thing: these exact words, normalised for whitespace and case, are on that
page (`verify.py:66-73`). Nothing broader.
...
at all (`quotes.py:1-7`). Typing `--text` yourself at the command line
bypasses that extraction step: whatever you pass is checked directly,
...
text verifies as `FLAG`, not `PASS` (`verify.py:68-71`). This is the common
case, not the exception: referee prose, whether typed by a person or
drafted by a model, mostly paraphrases the manuscript rather than quoting
it, and a paraphrase has no exact words to check against the page. `FLAG`
reports that honestly, as unverified, rather than treating a paraphrase as
a failed citation.
...
quotation is looked at (`verify.py:53-65`); a citation to a page that is
not in the document is a `FAIL`, never a `FLAG`, however little of it is
quoted. That ordering is what makes a `FLAG` claim safe to act on: unlike a
`FAIL`, its page is confirmed real, only its wording is unchecked — which
is why `review`'s question-and-answer loop records a `FLAG` claim into the
session's claim pool alongside every `PASS`, instead of discarding it
(`agent/loop.py:108-116`).
...
is a `FAIL`, not a `FLAG`, because the one thing a `FLAG` guarantees — that
the page exists — cannot be made here at all.
...
carry a claim (`quotes.py:12`). Separately, `verify` itself never returns
`PASS` for a claim whose text is fewer than four words; it verifies as
`FLAG` instead (`types.py:53`, `verify.py:68`). The four-word floor applies
to every claim regardless of where its text came from, including a
...
of `58%` never matches a paper's `5-8%` (`textnorm.py:70-72`).

When a `quote` or `page` claim fails, the evidence names the page's line
that most resembles the claimed text — a diagnostic to help you spot a
transcription slip, not a second chance to pass. It never changes the
verdict away from `FAIL` (`verify.py:10-29`).
...
by a number (`ingest.py:28-35`); the tutorial's fixture paper yields
figures 1 through 4.
...
(`verify.py:32-49`). The tutorial's fixture extracts twenty numeric ids,
and the contiguous run starting at 1 stops at 7 — so equation (18), though
it really was extracted, FAILs rather than PASSes:
...
This is `FAIL`, not `FLAG`, on purpose (`verify.py:88-97`): a `FLAG` would
join the claim pool and stay available to a draft, and an equation number
that extraction cannot vouch for has no business being citable. One
residual case sits outside this rule: a section-numbered label such as
`2.1` is not a plain integer, so the run does not apply to it — it passes
on bare existence in the extracted list instead, kept deliberately rather
than changed silently (`verify.py:81-87`).
...
(`ingest.py:37-44`). And an equation's body is never reconstructed at all:
only its number is extracted, so `verify` can confirm an equation exists
but never what it says (`ingest.py:25`).

## The two draft flags

A different check runs after `draft` or `editor` writes a report or
letter: every anchor the drafted prose cites is re-extracted and checked
against what the session actually established. This is where two flag
reasons come from — `not in verified pool` and `failed re-verification`
(`drafts.py:98-112`).
...
overwritten by a later `or-fetch` of a revised paper — re-fetching the same
submission number re-ingests it every time (`cli.py:283-284`) — so a claim
that verified against the original submission no longer verifies against
the replacement. Or the drafting model, writing the citation into its own
prose, can have altered the quoted words from what the pool actually
recorded — re-verification checks the words the draft wrote, not the
words originally stored, so an altered quotation is caught here.
...
words are a substring of a page — an `equation` claim only confirms that
extraction found that number somewhere in the document, never what the
equation says or whether it is correct. Checking a derivation stays your
own work: work it out by hand, or write a script and check it.
